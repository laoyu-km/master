...
上面代码中，`style`属性的值是一个 CSSStyleDeclaration 实例。这个对象所包含的属性与 CSS 规则一一对应，但是名字需要改写，比如`background-color`写成`backgroundColor`。改写的规则是将横杠从 CSS 属性名中去除，然后将横杠后的第一个字母大写。

<font color=red>**如果 CSS 属性名是 JavaScript 保留字，则规则名之前需要加上字符串`css`，比如`float`写成`cssFloat`。**</font>
...
<font color=red>**通过Element.style可以查看所有CSSStyleDeclaration实例能够设置的样式**</font>

```js
// html
// <div id="elDiv">Hello Jayden</div>
var oDiv = document.getElementById('elDiv');
...
<font color=red>**window.getComputedStyle(elem, 伪元素) 方法，就用来返回浏览器计算后得到的最终规则。它接受一个节点对象作为参数，返回一个 CSSStyleDeclaration 实例，包含了指定节点的最终样式信息。所谓“最终样式信息”，指的是各种 CSS 规则叠加后的结果。**</font>

<font color=red>**IE8及以下不支持**</font>
...
### window.getComputedStyle 的解决兼容性方法

- currentStyle: <font color="red">**可用来查询/修改外联或者内部样式表中的样式（仅IE、Opera）。 代表了在全局样式表、内嵌样式和 HTML 标签属性中指定的对象格式和样式。**</font>当使用currentStyle做条件判断是，要加上body,document.body.currentStyle，这样才能兼容上IE6,7。

```js
// 查看计算样式
// window.getComputedStyle[prop]
// window.getComputedStyle.porp
// IE8 及以下不支持，只支持 elem.currentStyle

// // window.getComputedStyle() 兼容性函数
// // 企业级写法
...
### **window.getComputedStyle().width 和 Element.offsetWidth

- window.getComputedStyle() 返回的是一个CSSStyleDeclaration实例，所以window.getComputedStyle.width 是实际的元素宽度不包含 padding，border， margin

- Element.offsetWidth 包含元素的 padding 和 border

- 实际使用中，动画等多使用window.getComputedStyle()的值，示例

  - <font color=red>**parseInt() 显示转化字符串为数字在实际开发中经常用**</font>
  ...
  <body>
      ...
      <script type="text/javascript">
          ...
          oDiv.onclick = function() {
             // var width = this.offsetWidth; // 使用offsetWidth 每次都会累加边框，要注意这个问题
              var width = parseInt(getStyle(this, 'width')); // 用 parseInt 来显示转换数字也是实际开发中常用的方法
...
<font color=red>**节点元素的`style`对象无法读写伪元素的样式，这时就要用到`window.getComputedStyle()`。JavaScript 获取伪元素，可以使用下面的方法。**</font>
...
- <font color=red>**操作伪元素，最好的方法是给element添加一个class**</font>
- <font color=red>**事实上，操作CSS最好使用的方法就是添加一个class，特别在公司尽可能这样写**</font>

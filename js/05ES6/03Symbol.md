...
注意，`Symbol`函数前不能使用`new`命令，否则会报错。这是因为生成的 Symbol 是一个原始类型的值，不是对象。也就是说，由于 Symbol 值不是对象，所以不能添加属性。基本上，<font color="red">它是一种类似于字符串的数据类型。</font>
...
上面代码中，`s1`和`s2`是两个 Symbol 值。如果不加参数，它们在控制台的输出都是`Symbol()`，不利于区分。有了参数以后，就等于为它们加上了描述，输出的时候就能够分清，到底是哪一个值。相同参数的`Symbol`函数的返回值是不相等的。
...
console.log(Symbol([1, 2])); // Symbol(1,2)
console.log(Symbol(function test() {
    var a = 1;
    return a;
})); // Symbol(function test() { var a = 1; return a; })
...
Object.defineProperty(obj, Symbol('3'), {
    value: 'nikki',
    writable: true,
    configurabale: true,
    enumerable: false
...
```js
// for...of 只作用于有部署了iterator的数据类型
// 迭代带有Symbol属性的obj
var arr = [Symbol('a'), Symbol('b')];
var obj2 = {
    '0': 'jayden',
    '1': 'alexis',
    [arr[0]]: 'foxxx',
...
Object.getPrototypeOf(obj2)[Symbol.iterator] = function() {
    var keys = Object.keys(this),
        symKeys = Object.getOwnPropertySymbols(this),
        arr = keys.concat(symKeys),
        len = arr.length,
        index = 0;
...
for (var key of obj2) {
    console.log(key.toString() + ' => ' + obj2[key]);
}
// 0 => jayden
// 1 => alexis
// Symbol(a) => foxxx
...
console.log(Object.values(obj)); // [jayden, alexis]
console.log(Object.entries(obj)); // [a, jayden], [b, alexis]
```
...
```js
// 返回对象自身的包含Symbol的所有属性，无论是否可枚举，但是不包含原型属性
console.log(Object.getOwnPropertyDescriptors(obj))
// {a: {…}, b: {…}, c: {…}, Symbol(1): {…}, Symbol(2): {…}, …}
```
...
```js
// 遍历自身可枚举且包含Symbol类型的属性
var tar = {};
Object.assign(tar, obj);
console.log(tar);
// {a: "jayden", b: "alexis", Symbol(1): "wicky", Symbol(2): "Lust"}
```
...
```js
// 遍历自身所有属性，无论是否可枚举且包含Symbol类型的属性

...
- 不同的是，<font color="red">**调用 Generator 函数后，该函数并不执行，返回的也不是函数运行结果，而是一个指向内部状态的指针对象，**</font> 也就是上一章介绍的遍历器对象（Iterator Object）。
- 必须调用遍历器对象的`next`方法，使得指针移向下一个状态。也就是说，每次调用`next`方法，内部指针就从函数头部或上一次停下来的地方开始执行，直到遇到下一个`yield`表达式（或`return`语句）为止。换言之，<font color="red">**Generator 函数是分段执行的，`yield`表达式是暂停执行的标记，而`next`方法可以恢复执行。**</font>
...
function* yieldArr(arr) {
    var _len = arr.length;
    for (var i = 0; i < _len; i++) {
        if (typeof arr[i] !== 'number') {
            yield yieldArr(arr[i]);
        } else {
            yield arr[i];
        }
    }
...
```js
// 第一个next()执行到第一个yield，后面表达式的值作为返回对象的value属性的值,第一个yield前面的语句都会执行
// next()的参数是上一个yield表达式的返回值。手动给定，如果不给的话默认是undefined
// yield所在行如果是一行语句或者表达式，语句和表达式在第二个next()执行的时候运行，同时可以获得第一个yield表达式的返回值
function* test() {
    console.log('first line');
    console.log(yield 'jayden');
    console.log('this is a generator test');
    var test = yield 'alexis';
    console.log('test', test);
    return yield 'the end';
}
...
// first line
// {value: "jayden", done: false}
// 13
// this is a generator test
// {value: "alexis", done: false}
// test 15
...
function* dataConsumer() {
    console.log('start');
    console.log(`No.1: ${ yield }`);
    console.log(`No.2: ${ yield }`);
    return 'the end';
}

var iter = dataConsumer();
...
上面代码使用`for...of`循环，依次显示 5 个`yield`表达式的值。这里需要注意，<font color="red">**一旦`next`方法的返回对象的`done`属性为`true`，`for...of`循环就会中止，且不包含该返回对象，所以上面代码的`return`语句返回的`6`，**</font>不包括在`for...of`循环之中。
...
function* objectEntries(obj) {
    let objKeyArr = Reflect.ownKeys(obj);
    for (var key of objKeyArr) {
        yield [key, obj[key]];
    }
}
...
Object.getPrototypeOf(obj)[Symbol.iterator] = function*() {
    // let objKeysArr = Reflect.ownKeys(this); 
    let objKeysArr = Object.getOwnPropertyNames(obj).concat(Object.getOwnPropertySymbols(obj)); // 与 Reflect.ownKeys() 效果相同
...
该参数会被`catch`语句接收，建议抛出`Error`对象的实例。
...
上面代码的错误，是用遍历器对象的`throw`方法抛出的，而不是用`throw`命令抛出的。后者只能被函数体外的`catch`语句捕获。
...
console.log('throw 内容捕获后，后续语句任然执行');

try {
    throw new Error('c');
    iter.throw(new Error('d'));
} catch (e) {
    co
```
...
iter.next();
iter.throw(new Error('this is a error'));
iter.next();
...
<font color="red">**这种函数体内捕获错误的机制，大大方便了对错误的处理。多个`yield`表达式，可以只用一个`try...catch`代码块来捕获错误。如果使用回调函数的写法，想要捕获多个错误，就不得不为每个函数内部写一个错误处理语句，现在只在 Generator 函数内部写一次`catch`语句就可以了。**</font>
...
var iter = gen(['jayden', 'alexis', 51, 'foxxx']);
...
function throwErrEndGenerator(iter) {
    console.log('funciton is begin');
    var v;

    try {
        v = iter.next();
        console.log('第一次执行next', v);
    } catch (e) {
        console.log('捕获错误', v);
    }
    try {
        v = iter.next();
        console.log('第二次执行next', v);
    } catch (e) {
        console.log('捕获错误', v);
    }
    try {
        v = iter.next();
        console.log('第三次执行next', v);
    } catch (e) {
        console.log('捕获错误', v);
    }
...
throwErrEndGenerator(gen());

// funciton is begin
// 第一次执行next {value: 1, done: false}
// throw an error
// 捕获错误 {value: 1, done: false}
// 第三次执行next {value: undefined, done: true}
// the end
```
...
上面代码中，遍历器对象`g`调用`return()`方法后，<font color="red">**返回值的`value`属性就是`return()`方法的参数`foo`**</font>。并且，Generator 函数的遍历就终止了，返回值的`done`属性为`true`，以后再调用`next()`方法，`done`属性总是返回`true`。
...
var gen2 = function*() {
    yield 'a';
    // yield gen1(); // 返回迭代器实例
    for (let atom of gen1()) {
        yield atom;
    }
    yield 'b';
}

for (var item of gen2()) {
    console.log(item);
}
// a
// 1
...
从语法角度看，<font color="red">**如果`yield`表达式后面跟的是一个遍历器对象，需要在`yield`表达式后面加上星号，表明它返回的是一个遍历器对象。这被称为`yield*`表达式。**</font>
...
### yield*后面的迭代器对象的Generator有return和没有return的区别
...
### Generator函数生成的遍历器是 Generator 函数的实例

Generator 函数总是返回一个遍历器，ES6 规定这个，也继承了 Generator 函数的`prototype`对象上的方法。
...
### 如果把`g`当作普通的构造函数使用this，并不会生效。

因为`g`返回的总是遍历器对象，而不是`this`对象。
...
再将`F`改成构造函数，就可以对它执行`new`命令了。<font color="red">**这样做gen生成的其他遍历器也会继承这些属性，需要注意**</font>
...
var obj1 = {
    ...
    step2: function() {
        console.log('obj1.step2');
    },
    step3: function() {
        console.log('obj1.step3');
    }
}

var obj2 = {
    step1: function() {
        console.log('obj2.step1');
    },
    step2: function() {
        console.log('obj2.step2');
    },
    step3: function() {
        console.log('boj2.step3');
    }
}

var obj3 = {
    step1: function() {
        console.log('obj3.step1');
    },
    step2: function() {
        console.log('obj3.step2');
    },
    step3: function() {
        console.log('obj3.step3');
    }
}
...
function* objGen(objs) {
    for (var i = 0; i < objs.length; i++) {
        var obj = objs[i];
        yield* stepGen(Object.values(obj));
    }
}

for (var item of objGen(objs)) {
    item();
}

// function scheduler(iter) {
//     var iterObj = iter.next(iter.value);
//     if (!iterObj.done) {
//         iter.value = iterObj.value
//         iter.value();
//         scheduler(iter);
//     }
// }

// scheduler(stepGen(Object.values(obj1)));
```

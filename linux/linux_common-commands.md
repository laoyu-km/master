...
| ----------------------------------------------------- | ---------------------------------------------- |
| useradd laoyu                                         | 创建⽤户                                       |
| userdel -r laoyu                                  | 删除⽤户                                       |
| usermod -g group_name user_name                       | 修改⽤户的组                                   |
| usermod -aG group_name user_name                      | 将⽤户添加到组                                 |
| usermod -s /bin/ksh -d /home/codepig –g dev laoyu | 修改⽤户laoyu的登录Shell、主⽬录以及⽤户组 |
...
| passwd                                                | 修改⼝令                                       |
| passwd laoyu                                      | 修改某⽤户的⼝令                               |
| w                                                     | 查看活动⽤户                                   |
| id laoyu                                          | 查看指定⽤户laoyu信息                      |
| last                                                  | 查看⽤户登录⽇志                               |
...
| cp dir/{file1,file2,file3}  /targetDir      | 复制多个文件到目标文件夹                                       |
...
| chmod -R 755 dir        | 递归修改文件夹权限, 子文件和子文件夹都改变|
...
| grep laoyu hello.txt      | 在⽂件hello.txt中查找关键词laoyu    |
| grep ^jayden hello.txt         | 在⽂件hello.txt中查找以jayden开头的内容  |
...
### 1. linux系统如何检查软件是否安装,Linux下检查是否安装过某软件包

1、rpm包安装的，可以用rpm -qa看到，如果要查找某软件包是否安装，用 rpm -qa | [grep](https://so.csdn.net/so/search?q=grep&spm=1001.2101.3001.7020) “软件或者包的名字”。

rpm -qa | grep [ruby](https://so.csdn.net/so/search?q=ruby&spm=1001.2101.3001.7020)
...
这一步会更新操作系统中所有软件到最新版，自行选择，非必要操作。

```bash
sudo yum -y update
```

### 3. centos7.6 YUM 源没有httpd资源的解决办法

```bash
 yum --disableexcludes=all list httpd
